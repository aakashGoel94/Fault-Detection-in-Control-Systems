import matplotlib.pyplot as plt
from scipy.integrate import odeint
from simple_pid import PID  # pip install simple_pid
import time
import pyfirmata
from pyfirmata import Arduino, util
from decimal import Decimal
import numpy as np
import csv
from gekko import GEKKO

port ='COM7'
board = Arduino('COM7')

it = util.Iterator(board)
it.start()

# arduino initialization
board.analog[0].mode = pyfirmata.INPUT
board.analog[1].mode = pyfirmata.INPUT
#board.digital[6].mode = pyfirmata.OUTPUT
motor=board.get_pin('d:6:p')

# create PID controller
# op = op_bias + Kc * e + Ki * ei + Kd * ed
#      with ei = error integral
#      with ed = error derivative
Kc = 2.0   # Controller gain
Ki = 0.03  # Controller integral parameter
Kd = 0.0  # Controller derivative parameter
pid = PID(Kc,Ki,Kd)
# lower and upper controller output limits
oplo = 0.0
ophi = 1.0
pid.output_limits = (oplo,ophi)
# PID sample time
pid.sample_time = 1.0

def tank(levels,t,pump,valve):
    levels[0] = board.analog[1].read()
    #print(levels[0])
    levels[1] = board.analog[0].read()
    h1 = max(0.0,levels[0])
    h2 = max(0.0,levels[1])
##    c1 = 0.06 # inlet valve coefficient
##    c2 = 0.04 # tank outlet coefficient
    Kp = 0.0525
    K1 = 1.5/67.8347
    K2 = 1.5/69.6996
    Kc1 = 0.87
    dhdt1 = Kp*pump - K1*h1
    dhdt2 = K1*Kc1*h1 - K2*h2
##    dhdt1 = c1 * (1.0-valve) * pump - c2 * np.sqrt(h1)
##    dhdt2 = c1 * valve * pump + c2 * np.sqrt(h1) - c2 * np.sqrt(h2)
##    A = 0.002827
##    a = 0.000007
##    Kp = 0.004
##    dhdt1 = -np.sqrt(2*h1)*a/A + Kp/A*pump
##    dhdt2 = np.sqrt(2*h1)*a/A - np.sqrt(2*h2)
    
    # overflow conditions
##    if h1>=0.6 and dhdt1>0.0:
##        dhdt1=0 
    if h2>=0.6 and dhdt2>0.0:
        dhdt2 = 0
    dhdt = [dhdt1,dhdt2]
    return dhdt

# Initial conditions (levels)
h0 = [0.0, 0.0]

# Time points to report the solution
tf = 1000
t = np.linspace(0,tf,tf+1)
# Inputs that can be adjusted
pump = np.zeros((tf+1))

# valve = 0, directly into top tank
# valve = 1, directly into bottom tank
valve = 0.0
# Record the solution
y = np.empty((tf+1,2))
y[0,:] = h0

plt.figure(1)
plt.ion()
plt.show()

# level setpoint (% full)
sp = np.zeros(tf+1)
sp[5:] = 0.4

# timing functions
tm = np.zeros(tf+1)
sleep_max = 1.01
start_time = time.time()
prev_time = start_time

# Simulate the tank step test
for i in range(tf):
    # PID control
    pid.setpoint=sp[i]
    pump[i] = pid(h0[1])
    motor.write(pump[i])
    #print(pump[i])

    # Specify the pump and valve
    inputs = (pump[i],valve)
    # Integrate the model
    h = odeint(tank,h0,[0,1],inputs)
    # Record the result
    y[i+1,:] = h[-1,:]
    # Reset the initial condition
    h0 = h[-1,:]

    # plot results
    plt.clf()    
    plt.subplot(2,1,1)
    plt.plot(t[0:i+1],y[0:i+1,0],'b-',label=r'$Tank1$ PV')
    plt.plot(t[0:i+1],y[0:i+1,1],'r--',label=r'$Tank2$ PV')
    plt.plot(t[0:i+1],sp[0:i+1],'k:',label=r'$Set Point$ SP')        
    plt.ylabel('Height (m)')
    plt.legend(loc='best')
    plt.subplot(2,1,2)
    plt.plot(t[0:i],pump[0:i],'k-',label='Motor')
    plt.legend(loc='best')
    plt.ylabel('Pump')
    plt.xlabel('Time (sec)')
    plt.pause(0.01)

    # Sleep time
    sleep = sleep_max - (time.time() - prev_time)
    if sleep>=0.01:
        time.sleep(sleep-0.01)
    else:
        time.sleep(0.01)

    # Record time and change in time
    ct = time.time()
    dt = ct - prev_time
    prev_time = ct
    tm[i+1] = ct - start_time


# Export data file
# reshape as column vectors
time_col = t.reshape(-1,1)
pump_col = pump.reshape(-1,1)
h2_col = y[:,1].reshape(-1,1)
my_data = np.concatenate((time_col,pump_col,h2_col), axis=1)
np.savetxt('pid_data.txt',my_data,delimiter=',')
